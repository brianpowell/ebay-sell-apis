...
module.exports = {
    ...
    */
    buildItem: function( item, availability, packageWeightAndSize, product ) {
    ...
    */
    buildAvailability: function ( ship_quantity ){
    ...
    */
    buildPackingAndWeight: function ( type, dimensions, weight ){
        ...
        if( !_.includes( enums.pack_types, String( type ).toUpperCase() ) ) {
         ...
         if( !_.includes( enums.dimensions, String( dimensions.unit ).toUpperCase() ) ) {
            debug("Unsupported dimensions.unit", dimensions.unit)
            return false;
        }

        if( !_.includes( enums.weights, String( weight.unit ).toUpperCase() ) ) {
    ...
    */
    buildProduct: function ( product ){
        ...
        // Check enum values
        if( !product.title || !product.brand || !product.description ) {
            debug("Missing Required Param (buildProduct)!!!");
            debug(" > Title", product.title);
            debug(" > Brand", product.brand);
