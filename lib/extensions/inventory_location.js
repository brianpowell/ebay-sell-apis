    ...
    */
    buildLocation: function( location, address, locationType, operatingHours, specialHours ) {
        ...
        if( !location.name || !location.title ) {
            ...
            debug(" > title", location.title);
            return false;
        }

        if( _.isEmpty( address ) && _.isEmpty( location.address ) ) {
            debug("Missing Required Param (buildLocation)!!!");
            debug(" > address", address);
            debug(" > location.address", location.address);
            return false;
        }
    ...
    */
    buildHour: function _buildHour( type, day_val, intervals ) {
        ...
        if ( type == "operating" ) {
	        ...
	        hours_day["dayOfWeekEnum"] = day_val;
    ...
    // Shorthand Hours
    buildOperatingHours: 	function _buildOperatingHours( day_val, intervals ) { return _buildHour( "operating", day_val, intervals ); },
    buildSpecialHours: 		function _buildSpecialHours( day_val, intervals ) { return _buildHour( "special", day_val, intervals ); },
