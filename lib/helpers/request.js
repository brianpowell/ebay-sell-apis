...
Request.prototype.getAuthUrl = function( client_id, redirect, in_scopes ) {
    
    let req = this;

    // Check required
    if( !client_id || !redirect ) {
         debug("getAuthUrl > missing requires inputs", client_id, redirect );
        return false;
    }
    ...
    return req.auth + "?" + qs.stringify( query );
}
